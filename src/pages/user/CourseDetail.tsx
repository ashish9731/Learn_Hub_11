...
export default function CourseDetail() {
  ...
  // Check if final quiz has been passed
  const checkFinalQuizPassed = () => {
    // Check if any course quiz attempt has been passed
    return quizAttempts.some(attempt => 
      attempt.course_quizzes && attempt.passed === true
    );
  };
                          ...
                          <div key={pdf.id} className="border border-gray-700 rounded-lg p-4 hover:shadow-md transition-shadow bg-gray-800">
                            <div className="flex items-start">
                              ...
                              <div className="ml-3 flex-1">
                                ...
                                <button
                                  onClick={() => {
                                    // Use Supabase URL with inline content disposition to force browser preview
                                    let url = pdf.pdf_url;
                                    if (url && url.includes('supabase')) {
                                      // Add response-content-disposition=inline parameter to force browser preview
                                      if (!url.includes('response-content-disposition')) {
                                        url = url + (url.includes('?') ? '&' : '?') + 'response-content-disposition=inline';
                                      }
                                    }
