...
export default function Analytics() {
  ...
  const loadSupabaseData = async () => {
    try {
      ...
      const [companiesData, coursesData, podcastsData, pdfsData, usersData, userCoursesData] = await Promise.all([
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Created</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Content Items</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <RePieChart>
                      <Pie
                        ...
                        cy="50%"
                        labelLine={false}
                        label={({ area, percent }) => `${area} ${(percent ? percent * 100 : 0).toFixed(0)}%`}
                        outerRadius={80}
                        fill="#8884d8"
                        dataKey="value"
                      >
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Role</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Company</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Learning Hours</th>
  ...
  const getCompanyData = () => {
    return supabaseData.companies.map((company: Company) => ({
      id: company.id,
      name: company.name,
      userCount: supabaseData.users.filter((user: User) => user.company_id === company.id).length,
      courseCount: supabaseData.courses.filter((course: Course) => course.company_id === company.id).length,
  ...
  const calculateEngagementRate = (companyId: string) => {
    const companyUsers = supabaseData.users.filter((user: User) => user.company_id === companyId);
